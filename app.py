...
# 4. Flecha Norte y Leyenda (Minimalista)
FloatImage("https://raw.githubusercontent.com/sjauregui/folium_examples/master/north_arrow.png", bottom=85, left=5).add_to(m)
legend_html = '''<div style="position: fixed; bottom: 50px; left: 50px; z-index:9999; font-size:12px; background:white; padding:10px; border-radius:5px; border:1px solid #ccc;">
      <b>Leyenda:</b><br>
      <i class="fa fa-circle" style="color:green"></i> Alerce<br>
      <i class="fa fa-circle" style="color:red"></i> Ranita<br>
      <i class="fa fa-circle" style="color:brown"></i> Chucao
     </div>'''
...
# 6. Capas (Con estilo forzado para asegurar visibilidad)
folium.GeoJson(reserva, name="Reserva", style_function=lambda x: {'fillColor': 'transparent', 'color': '#004d00', 'weight': 3, 'opacity': 1}).add_to(m)
folium.GeoJson(rios, name="Ríos", style_function=lambda x: {'color': '#00BFFF', 'weight': 2, 'opacity': 1}).add_to(m)
...
for _, row in especies[especies['common_name'].isin(seleccion)].iterrows():
    ...
    color = 'green' if 'Alerce' in nombre else ('red' if 'Ranita' in nombre else 'brown')
    
    # Popup HTML
    html = f'<div style="width:150px; font-family:sans-serif;"><h4>{nombre}</h4><img src="{url}" style="width:100%; border-radius:5px;"></div>'
